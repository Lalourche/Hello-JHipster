...
export default (state: StepState = initialState, action): StepState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_STEPS):
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_STEP):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.SEARCH_STEPS):
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_STEP):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_STEPS):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_STEP):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
const apiSearchUrl = 'api/_search/steps';

// Actions

export const getSearchEntities: ICrudSearchAction<IStep> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_STEPS,
  payload: axios.get<IStep>(`${apiSearchUrl}?query=${query}`),
});
...
export const partialUpdate: ICrudPutAction<IStep> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_STEP,
